...
class SQLiteDatabaseRepository(DatabaseRepository):
    ...
    async def _ensure_schema(self):
        ...
        async with self._get_connection() as conn:
            ...
            if not await cursor.fetchone():
                logfire.warning("Database schema not initialized. Run migrations first.")
        ...
        async with self._get_connection() as conn:
            cursor = await conn.execute("""
                SELECT key, value, category, description, 
                       display_name, is_secret, updated_at 
    ...
    async def upsert_setting_record(self, setting_data: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._get_connection() as conn:
            ...
            """, (
                ...
                setting_data.get('display_name'),
                setting_data.get('is_secret', False),
                datetime.now().isoformat()
            ))
    ...
    async def list_sources_with_pagination(
        ...
        async with self._get_connection() as conn:
            ...
            return (self._rows_to_list(rows), total_count)
    ...
    async def upsert_source(self, source_data: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._get_connection() as conn:
            ...
            # Prepare JSON fields
            crawl_config = json.dumps(source_data.get('crawl_config', {}))
            
            await conn.execute("""
                INSERT OR REPLACE INTO archon_sources (
                    source_id, source_url, source_display_name,
                    knowledge_type, crawl_status, last_crawled_at,
            ...
            """, (
                ...
                source_data.get('knowledge_type', 'documentation'),
                source_data.get('crawl_status', 'pending'),
                source_data.get('last_crawled_at'),
                crawl_config,
