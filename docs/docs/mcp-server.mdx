...
Archon MCP server now defaults to **Streamable HTTP** transport for maximum compatibility with Claude Code CLI and other modern MCP clients. The server listens on `http://localhost:8051/mcp` by default. You can still run in SSE mode by setting `MCP_TRANSPORT=sse` (or passing `sse` on the command line) when you need backwards-compatible clients.
...
> **Need SSE or stdio instead?**
> - Set `MCP_TRANSPORT=sse` to reuse the existing SSE endpoints (`/sse`, `/messages/`).
> - Run `python -m src.mcp_server.mcp_server stdio` (or `MCP_TRANSPORT=stdio`) to enable stdio transport.
